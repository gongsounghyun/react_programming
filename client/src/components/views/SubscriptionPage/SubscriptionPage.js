...
function SubscriptionPage() {
  ...
  const renderVideoCards = Video.map((video, index) => {
    ...
    return (
      <Col style={{ marginBottom: "8px" }} key={index} lg={6} xs={12}>
      ...
      <Col style={{ marginBottom: "8px" }} key={index} lg={6} xs={12}>
        ...
        >
          <a href={`image/${image.docid}`}>
            <img
              style={{ width: "auto", height: "180px", textAlign: "center" }}
              alt="thumbnail"
              src={`${image.url}`}
            />
          </a>
  ...
  return (
    <div style={{ width: "85%", margin: "3rem auto" }}>
      ...
      <Search
        ...
        onSearch={onSearch}
      />
      <Title level={2}>게시물</Title>
      <hr />
      <Title level={2}>이미지</Title>
      <Row gutter={16}>{renderImageCards}</Row>
      <Title level={2}>비디오</Title>
      <Row gutter={16}>{renderVideoCards}</Row>
